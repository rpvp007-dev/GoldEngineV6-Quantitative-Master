...
bool ExecuteNewOrderPlacement(datetime currentBarTime, bool isMidCandle = false)
{
   ...
   string prompt = StringFormat(
      ...
      "Instructions: 1. You are highly encouraged to swing-trade using LONG_TERM horizon when you see daily trend direction or clear wick rejections at key levels. 2. During strong trends (ADX > 25.0 and price above/below EMA200/VWAP), execute a LONG_TERM trade in the direction of the trend (e.g., using VOLUME_BREAKOUT or DONCHIAN_BREAKOUT) to capture a major multi-hour move. 3. If recent closed trades show consecutive losses (negative profits), you MUST raise your conviction requirement (be extremely defensive, only enter on A+ setups with >80 conviction). 4. If free margin is low, prioritize holding capital and issue 'HOLD'. 5. Do NOT execute SELL trades when the price is close to (within 5 points of) an untested GNN Support floor (Aqua line), and do NOT execute BUY trades when the price is close to (within 5 points of) an untested GNN Resistance ceiling (Golden line). 6. CRITICAL SL SPACING RULE: If you enter a BUY trade at a shallow pullback level (like a Fair Value Gap or Order Block) and there is a GNN Support floor (Aqua line) below it, you MUST place your stop_loss_price at least 3 to 5 USD below that GNN Support floor (not right on it), so that a full test of the support floor does not stop you out. Conversely, for a SELL trade entered on a bearish pullback, your stop_loss_price must be at least 3 to 5 USD above the Golden resistance ceiling. 7. SIDEWAYS PING-PONG RULE: When you detect the market is ranging sideways in a tight micro-channel (e.g. oscillating between 4053 and 4058 with low ADX < 20), you are highly encouraged to select SCALPING strategy and SHORT_TERM horizon to ping-pong the range: place a BUY when price is near the bottom of the range (e.g. 4053) setting your take_profit_price at the top of the range (e.g. 4058), and place a SELL when price is near the top of the range setting your take_profit_price at the bottom of the range. Set the stop_loss_price and take_profit_price explicitly in your JSON response to target these exact boundaries to make multiple quick trades. 8. STRICT RANGE ENTRY LIMITS: If the market is inside a sideways range or channel (e.g. GNN floor is 4051 and GNN ceiling is 4067), you are STRICTLY FORBIDDEN from issuing a BUY unless the current Ask price is within 2.0 USD of the bottom support floor (Aqua line), and you are STRICTLY FORBIDDEN from issuing a SELL unless the current Bid price is within 2.0 USD of the top resistance ceiling (Golden line). Entering trades in the middle of the range (such as buying at 4058 or 4060 when support is down at 4051) is a critical error that leads to instant losses on deep retracements. You must be patient: wait for the price to fully touch/wick test the deep GNN lines before executing any range trades. 9. SUPPORT CLUSTER STOP PLACEMENT: When analyzing untested price magnets, if you see multiple support floors (Aqua lines) clustered close together (e.g., within 5 to 8 USD of each other), you must treat the second, deeper Aqua line as the true structural floor. Place your stop_loss_price at least 3 to 5 USD below the second (deeper) support floor (not the first one), so that a normal liquidity sweep of the minor support does not stop you out. Conversely, if there are multiple resistance ceilings (Golden lines) clustered close together, place your stop_loss_price 3 to 5 USD above the second, higher ceiling. "+
