      ...
      if(PositionGetSymbol(i) == _Symbol)
      {
         if(PositionGetInteger(POSITION_MAGIC) == InpMagicNumber)
         {
            ...
            else if(type == POSITION_TYPE_SELL)
            {
             ...
             bool isScalpTrade = (StringFind(comment, "Scalp") >= 0);
            if(isScalpTrade)
            {
...
bool ExecuteNewOrderPlacement(datetime currentBarTime, bool isMidCandle = false)
{
   ...
   string prompt = StringFormat(
      ...
      "2. Intermediate Confluences (Phase 2): Check VWAP deviation, EMA slope momentum, RSI momentum, ATR volatility sizing, and Fibonacci retracement confluence. "+
      "3. Advanced Price Action & Institutional Flow (Phase 3): Scan for mitigation blocks, breaker blocks, rejection blocks, and check for liquidity sweeps/stop hunts. Combine H1 bias with Daily/Weekly bias alignment. "+
      ...
      "5. Professional Risk & Position Management (Phase 5): Audit target stop loss (placed at least 3-5 USD beyond GNN/structure boundary) and take profit targets. Perform position sizing and leverage audits. "+
